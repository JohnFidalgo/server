			...
			var afterCall = function(data, statusText, xhr) {
				...
				if (xhr.status === 200 && data) {
					...
					if(data.phpSupported && data.phpSupported.version.substr(0, 3) === '5.6') {
						messages.push({
							msg: t('core', 'You are currently running PHP 5.6. The current major version of Nextcloud is the last that is supported on PHP 5.6. It is recommended to upgrade the PHP version to be able to upgrade to Nextcloud 14.', {version: data.phpSupported.version}),
							type: OC.SetupChecks.MESSAGE_TYPE_INFO
						});
					}
